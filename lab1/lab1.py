...
sibling_rule = IF( AND('parent (?x) (?y)', 'parent (?x) (?z)', NOT('same-identity (?y) (?z)')),
               THEN( 'sibling (?y) (?z)'))

sibling_rule2 = IF( AND('parent (?x) (?y)', 'parent (?x) (?z)', NOT('same-identity (?y) (?z)')),
               THEN( 'sibling (?z) (?y)'))
...
cousin_rule = IF( AND('parent (?a) (?b)', 'parent (?c) (?d)', ('sibling (?a) (?c)')),
              THEN('cousin (?b) (?d)'))

grandparent_rule = IF( AND('parent (?a) (?b)', 'parent (?b) (?c)'),
                   THEN('grandparent (?a) (?c)'))

grandchild_rule = IF( AND('parent (?a) (?b)', 'parent (?b) (?c)'),
                   THEN('grandchild (?c) (?a)'))

# Then, put them together into a list in order, and call it
# family_rules.
# family_rules = [same_identity_rule, sibling_rule, brother_rule, sister_rule, mother_rule, 
#                 father_rule, son_rule, daughter_rule, grandparent_rule]    

family_rules = [same_identity_rule, sibling_rule, brother_rule, sister_rule, mother_rule, father_rule, son_rule, daughter_rule, grandparent_rule, grandchild_rule, cousin_rule]                 
...
WHAT_I_FOUND_INTERESTING = 'backward'
WHAT_I_FOUND_BORING = 'first stuff'
