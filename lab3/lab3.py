...
def alpha_beta_search(board, depth,
    ...
    # best_val = None
    # print 'starting alpha-beta search with current player:', board.get_current_player_id()
    alpha = (-1000,-1,board)
    beta = (1000,-1,board)
    nodeType = 'max'
    ...
    for move, new_board in get_next_moves_fn(board):
        ...
        #print 'performing minimax on: ', new_board, ', depth :', 4-depth
        val = alpha_beta_find_board_value(nodeType,alpha[0],beta[0],new_board, depth-1, eval_fn,
                                            get_next_moves_fn,
                                            is_terminal_fn)
        # print 'got board value: ', val , 'move :', move
        if val > alpha[0]:
...
def alpha_beta_find_board_value(nodeType,alpha,beta,board, depth, eval_fn,
    ...
    if is_terminal_fn(depth, board):
        eval_val = eval_fn(board)
        #print 'leaf node value :', eval_val
        if nodeType == 'max':
            return eval_val
        else: return -eval_val
    ...
    for move, new_board in get_next_moves_fn(board):
        #print 'performing minimax on: ', new_board, ', depth :', 4-depth
        val = alpha_beta_find_board_value(nodeType,alpha,beta,new_board, depth-1, eval_fn,
                                            get_next_moves_fn, is_terminal_fn)
        #print 'got board value: ', val , 'depth: ', 4 - depth,'node type:',nodeType
        ...
        if nodeType == 'max':
          ...
          if val < beta:
            beta = val
        if alpha >= beta:
          #print 'alpha:',alpha,'>=','beta',beta,'at depth',4-depth,';pruning'
          break;      
...
HOW_MANY_HOURS_THIS_PSET_TOOK = "10000"
